...
test-dev-cluster:
	...
	[ ! -d test ] || (cd test; docker compose exec  --privileged masterless salt-call --local state.apply steamcmd -l debug)

test-show-sls-dev-cluster:
	@echo "Test dev Cluster"
	[ ! -d test ] || (cd test; docker compose exec  --privileged masterless salt-call --local state.show_sls steamcmd)

render-sls:
	salt-call --local state.show_sls steamcmd
